...
The system MUST use Vitest with `@vitest/coverage-v8` and MUST configure coverage thresholds (80% lines, functions, branches, statements) natively in `vitest.config.ts`, such that `vitest run --coverage` exits non-zero when any threshold is not met.

#### Scenario: Coverage above threshold passes

- GIVEN the placeholder module has a unit test achieving at least 80% on all four coverage metrics
