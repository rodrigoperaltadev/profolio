...
- [x] 3.1 Create `src/pages/admin/login.astro` — GET renders a plain HTML form, no client-side framework runtime; shows a `?error=` message when present
- [x] 3.2 Create `src/pages/admin/api/login.ts` — POST: `isLockedOut()` check → `timingSafeStringEqual()` secret check → wrong: `recordFailedAttempt()` + 303 redirect with error → correct: `clearLockout()` + `issueSession()` + `cookies.set()` (`HttpOnly`, `Secure`, `SameSite=Strict`, `path: "/admin"`, `maxAge`) + 303 redirect to `/admin`
- [x] 3.3 Modify `scripts/verify-admin-server.mjs` **in place** (no 6th script) — replace every Basic Auth request with: (a) POST correct secret → follow redirect → GET `/admin` with the cookie → allowed/non-401; (b) 5x POST wrong secret from the same simulated client → 6th attempt denied without evaluating the secret (lockout proof); (c) full mode, `ADMIN_ACCESS_TOKEN` unset → fail-closed; (d) local-fallback mode (no GitHub env vars) → no gate. Also discovered and worked around a real integration gotcha: Astro's built-in `security.checkOrigin` (default-on for server output) rejects form POSTs missing an `Origin` header as cross-site — the script now sends `origin: BASE_URL` on the login POST to simulate a real browser
- [x] 3.4 Copy `SECURITY.md` content **verbatim** from design.md's "SECURITY.md (structure + real content)" section into repo-root `SECURITY.md` — do not re-author
- [x] 3.5 Verify: **actually run** `npm run build` then `npm run verify:admin-server` against the real server (not author-and-assume) — exits 0; all 7 proofs pass (local-fallback bypass, fail-closed, correct-secret login+cookie, session-cookie-grants-access, 5 failed attempts recorded, 6th attempt lockout, old Basic Auth dead)
- [x] 3.6 Verify the old Basic-Auth mechanism is genuinely removed: manually ran the built server and sent `curl -u admin:<ADMIN_ACCESS_TOKEN>` against `/admin` — confirmed `303` redirect to `/admin/login`, NOT authenticated (also covered by the automated script's `proveOldBasicAuthMechanismIsDead()`)
- [x] 3.7 Verify the cookie API empirically: manually ran the built server, `curl -i` with the required `Origin` header against `/admin/api/login` — real `Set-Cookie` header confirmed: `admin_session=...; Max-Age=86400; Path=/admin; HttpOnly; Secure; SameSite=Strict` — all four flags genuinely present, `Max-Age=86400` = 24h
- [x] 3.8 Re-verify both mode-dependent behaviors on the real running server: confirmed via both the automated script (`proveLocalFallbackBypass`/`proveFailClosedWithoutToken`) and the manual curl checks above — full mode + unset `ADMIN_ACCESS_TOKEN` denies all `/admin/**`; local-fallback mode (no GitHub env vars) has no gate at all
- [x] 3.9 Review `SECURITY.md` against the final implementation: confirmed `SESSION_TTL_MS = 24 * 60 * 60 * 1000` (24h) in `src/config/admin-session.ts` matches `SECURITY.md`'s "24 hours" claim exactly — no update needed
- [x] 3.10 Verify: `npm run test` (156/156, 100% coverage all 4 metrics), `npm run typecheck`, `npm run lint`, `npm run build` all exit 0 — reconfirmed from a clean state (`rm -rf .astro dist node_modules/.cache`)
- [x] 3.10b **Regression found and fixed, not in the original task list:** `scripts/verify-env-file-loading.mjs` (from the archived `env-wizard` change, #7) still asserted the OLD Basic-Auth 401/200 semantics — it failed for real (`expected 401 with wrong Basic Auth credentials, got 200`) because `fetch()` without `redirect: "manual"` silently followed the new gate's 303 into the login page. Fixed by rewriting its proof to use `redirect: "manual"` + a real login POST (mirroring `verify-admin-server.mjs`'s pattern) instead of a Basic Auth header. Full clean-state run of all 5 `verify-*.mjs` scripts + test/typecheck/lint/build confirmed green after the fix — no other pre-existing script regressed.
- [x] 3.11 Commit as one work unit; open PR3 → PR2 branch (final child; cascades to tracker → main)
